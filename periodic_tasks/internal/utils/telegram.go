...
// SendTelegramMessage sends a text message to a Telegram user
...
// SendTelegramDocument sends a document/file to a Telegram user
func SendTelegramDocument(botToken string, chatID int64, filename string, fileData []byte, caption string) error {
	if botToken == "" {
		return fmt.Errorf("bot token is empty")
	}
